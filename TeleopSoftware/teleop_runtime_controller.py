...
freedrive_state: dict[str, bool] = {}
gripper_state: dict[str, bool] = {}
BUTTON_NAMES = ["connect", "freedrive", "gripper", "home"]
...
def connect_dashboard(arm, fields, URs, colors):
...
def connect_arm(arm, fields, URs, colors, control_modes, enable_control):
    if URs.init_arm(arm, enable_control=enable_control):
        ...
        if arm in freedrive_state:
            del freedrive_state[arm]
            toggle_freedrive(arm, fields, URs, colors, control_modes)
        fields[arm]["connect"].config(background="grey")


def reset_estop(arm, fields, URs, colors, control_modes, pubs, enable_control):
    ...
    connect_arm(arm, fields, URs, colors, control_modes, enable_control)
...
def toggle_freedrive(arm, fields, URs, colors, control_modes):
    ...
    if arm in freedrive_state:
        ...
        freedrive_state[arm] = True


def toggle_gripper(arm, URs):
    if arm in gripper_state:
        print(arm + ": Close Gripper")
        URs.get_gripper(arm).set(255)
        del gripper_state[arm]
    else:
        print(arm + ": Open Gripper")
        URs.get_gripper(arm).set(0)
        gripper_state[arm] = True


def home_arm(arm, fields, URs, colors, homes, control_modes, pos=None):
    control_modes[arm] = None
    print(arm)
    for button in fields[arm]["run_buttons"]:
        button.config(bg=colors[arm])
...
def invert_space_mouse(arms, fields, control_modes):
